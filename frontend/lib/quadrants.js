...
//
// 중요: quadrant 값은 데이터 쪽에서 이미 계산해서 내려주는 필드다.
// 프론트에서는 이 값을 색상/라벨에 매핑만 하고, 좌표로부터 재계산하지 않는다.
// (차트의 X=50 / Y=50 배경 4분할선은 어디까지나 시각적 참고선이며,
//  실제 색상은 각 점의 quadrant 필드를 따른다.)
//
// 직군 필터가 걸리면 y축이 그 직군 안에서의 백분위로 바뀌므로 사분면도 함께
// 바뀐다. 그 경우에도 프론트가 계산하지는 않는다 — 직군별 값은 미리 계산돼
// roleBreakdown[].quadrant에 실려 오고, lib/roles.js가 그중 하나를 고를 뿐이다.
//
// 두 축의 분류 기준선. 생태계 점수와 채용 수요 모두 0~100 정규화 값이라
// 50을 중앙으로 쓴다. 실측 200개 기술 기준으로 65 / 34 / 33 / 68로 갈려 네
// 사분면이 고르게 차고, 차트가 그리는 50% 십자선과 시각적으로도 일치한다.
// 값 자체는 데이터 생성기가 quadrant를 계산할 때 쓰는 기준이며(위 파일 참고),
// 프론트는 아래 매핑에만 쓴다.
